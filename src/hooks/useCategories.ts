...
export function useCategories() {
  ...
  const createCategory = async (name: string, parent_id: string | null = null) => {
    ...
    try {
      ...
      const insertPromise = supabase.from('categories').insert({ name, slug, parent_id }).select().single()
  ...
  const updateCategory = async (id: string, name: string, parent_id: string | null = null) => {
    const slug = name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')

    if (!isSupabaseConfigured) {
      const updated = categories.map((c: Category) => (c.id === id ? { ...c, name, slug, parent_id } : c))
      setCategories(updated)
      localStorage.setItem(LOCAL_CATS_KEY, JSON.stringify(updated))
      return { data: { id, name, slug, parent_id }, error: null }
    }
