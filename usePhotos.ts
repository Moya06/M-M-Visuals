      ...
      try {
        ...
        if (!cancelled) {
          if (!sbError && data && Array.isArray(data)) {
            ...
            console.warn('Supabase fetch issue (maybe RLS policies?):', sbError)
            // Even if it failed due to RLS, we fallback to local just in case they are local testing
            // BUT it is a warning now so they know why it fails.
            let list = await getLocalPhotos()
        ...
        if (!cancelled) {
          console.error('Catch error in fetchPhotos:', err)
